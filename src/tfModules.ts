...
class ElasticContainerServiceArc extends TFModuleStack {
  constructor(scope: Construct, id: string) {
    ...
    const filterType = VariableType.list(
      VariableType.object({
    ...
    const securityGroupFilters = new TFModuleVariable(
      this,
    ...
    const securityGroupFiltersIterator =
      DynamicListTerraformIterator.fromList(securityGroupFilters)
    const subnetFiltersIterator =
      DynamicListTerraformIterator.fromList(subnetFilters)
